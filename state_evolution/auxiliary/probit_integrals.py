...
# == ALTERNATIVE FUNCTIONS TO COMPUTE MHAT UPDATE, RELY ON THE DERIVATIVE
...
def f_mhat_plus_der(ξ, M, Q, V, Vstar):
    ω = np.sqrt(Q)*ξ
    ωstar = (M/np.sqrt(Q))*ξ    
    # derivative w.r.t x, need the derivative of 

    λstar_plus = root_scalar(lambda x : moreau_prime(x, 1, ω, V),
                             fprime = lambda x : moreau_second(x, 1, ω, V),
                             x0 = ω).root
    return np.exp(-ωstar**2/(2*Vstar))*(λstar_plus - ω)

def f_mhat_minus_der(ξ, M, Q, V, Vstar):
    ω = np.sqrt(Q)*ξ
    ωstar = (M/np.sqrt(Q))*ξ    
    # derivative w.r.t x, need the derivative of 
    λstar_minus = root_scalar(lambda x : moreau_prime(x, -1, ω, V),
                              fprime = lambda x : moreau_second(x, -1, ω, V),
                              x0 = ω).root
    return np.exp(-ωstar**2/(2*Vstar))*(λstar_minus - ω)

def integrate_for_mhat_der(M, Q, V, Vstar):
    I1 = quad(lambda ξ: f_mhat_plus_der(ξ, M, Q, V, Vstar) * gaussian(ξ), -10, 10, limit=500)[0]
    I2 = quad(lambda ξ: f_mhat_minus_der(ξ, M, Q, V, Vstar) * gaussian(ξ), -10, 10, limit=500)[0]
    return (I1 - I2)*(1/np.sqrt(2*np.pi*Vstar))
